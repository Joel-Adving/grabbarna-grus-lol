...
export default function GrusGrabb({ params }: any) {
  const { matchHistory, summoner, loading } = useGetMatchHistory(params?.name)
      ...
      {!loading && (
        <div className="min-h-screen bg-gradient-to-b from-background-darkest via-background-background to-background">
          <div className="container px-3 pt-3 mb-6 space-x-8 text-text font-BeaufortBold md:mb-0 sm:px-0">
            ...
            {summoner && summoner.rankedStats.length > 0 && <button onClick={() => setFilter('RANKED')}>RANKED</button>}
